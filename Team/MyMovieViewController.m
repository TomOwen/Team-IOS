...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //NSString *mediaType = [[SharedObject sharedTeamData] getMediaType];
    ...
    //NSString *imageURL = [[NSString alloc] initWithFormat:@"%@%@-%@-%@.%@",serverURL,patientID,newDateString,lesionText,mediaType];
    //NSLog(@"url = %@",imageURL);

    
    
    
    //NSURL *movie = [[NSURL alloc] initWithString:@"http://www.whereryou.webuda.com/gallery/143-120811-1.mov"];
    NSURL *movie = [[NSURL alloc] initWithString:imageURL];
    //NSLog(@"b4play url is %@",movie);
    [self playMovieStream:movie];
}
