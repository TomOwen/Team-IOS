...
#import "StudyGraphDisplay.h"


@interface StudyGraphDisplay ()

@end

@implementation StudyGraphDisplay
...
- (void)viewDidLoad
{
    ...
    NSString *study_id = [[SharedObject sharedTeamData] getStudyid];
    ...
    NSString *serverURL = [[NSString alloc]initWithFormat:@"http://%@:%@@%@/graphstudy.php?teamid=%@&study_id=%@&user_name=%@",dbuser,dbpassword,dbserver,teamid,study_id,myuser];
    //NSLog(@"url=%@",serverURL);
...
- (IBAction)printDetails:(UIBarButtonItem *)sender {
    ...
    printInfo.jobName = @"Study Scan Graph";
